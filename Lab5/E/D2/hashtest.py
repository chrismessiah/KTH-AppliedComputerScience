...
from new_new_hashtabell import *

def skapaAtomlista():
    """Returnerar en lista med atomernas key och value"""
    print("\n-------------------------------------------------------")
...
def lagraHashtabell(atomlista):
    ...
    for element in atomlista:
        ...
        nyAtom = Node(key, float(value))
        hashtabell.put(key, nyAtom)
    return hashtabell
...
def allaAtomerFinns(hashtabell, atomlista):
    ...
    for kontrollAtom in atomlista:
        key, value = kontrollAtom.split()
        value = float(value)
        try:
            hashadAtom = hashtabell.get(key)
            if hashadAtom.value != value:
                print(key, "har fel value.")
            else:
                antal += 1
        except KeyError:
            print(key, "fanns inte med i hashtabellen.")
            OK = False
...
def knasAtomFinnsInte(hashtabell):
    ...
    knaskey = "Zz"
    print("\n-------------------------------------------------------")
    print( " * Testar att hitta en atom som inte finns:", knaskey, "...")
    try:
        atom = hashtabell.get(knaskey)
        print(knaskey, "fanns med i hashtabellen.")
        return False
    except KeyError:
        print(knaskey, "fanns inte med i hashtabellen.")
        return True
