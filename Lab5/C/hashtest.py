...
from new_new_hashtabell import Hashtabell
...
def skapaAtomlista():
    """Returnerar en lista med atomernas name och weight"""
    print("\n-------------------------------------------------------")
...
def lagraHashtabell(atomlista):
    ...
    for element in atomlista:
        ...
        nyAtom = Atom(name, float(weight))
        hashtabell.put(name, nyAtom)
    return hashtabell
...
def allaAtomerFinns(hashtabell, atomlista):
    ...
    for kontrollAtom in atomlista:
        name, weight = kontrollAtom.split()
        weight = float(weight)
        try:
            hashadAtom = hashtabell.get(name)
            if hashadAtom.weight != weight:
                print(name, "har fel weight.")
            else:
                antal += 1
        except KeyError:
            print(name, "fanns inte med i hashtabellen.")
            OK = False
...
def knasAtomFinnsInte(hashtabell):
    ...
    knasname = "Zz"
    print("\n-------------------------------------------------------")
    print( " * Testar att hitta en atom som inte finns:", knasname, "...")
    try:
        atom = hashtabell.get(knasname)
        print(knasname, "fanns med i hashtabellen.")
        return False
    except KeyError:
        print(knasname, "fanns inte med i hashtabellen.")
        return True
